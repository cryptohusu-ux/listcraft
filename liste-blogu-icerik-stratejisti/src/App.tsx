...
export default function App() {
    ...
    if (localData) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Robust filter to verify each strategy has the required properties
          const validStrategies = parsed.filter((s: any) => 
            s && 
            typeof s === "object" && 
            Array.isArray(s.items) && 
            Array.isArray(s.titleSuggestions) &&
            s.titleSuggestions.length > 0
          );

          if (validStrategies.length > 0) {
            setSavedStrategies(validStrategies);
            const active = validStrategies[0];
            setCurrentStrategy(active);
            setSelectedTitle(active.selectedTitle || active.titleSuggestions?.[0] || "");
          } else {
  ...
  const handleTitleChange = (title: string) => {
    ...
    updateSavedStrategies(updated);
    setCurrentStrategy(prev => (prev ? { ...prev, selectedTitle: title } : prev));
  };
